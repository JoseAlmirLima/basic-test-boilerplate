...
class FiltersView {
  ...
  renderFilters() {
    ...
    filters.map((item, index) => {
      ...
      labelEl.innerHTML = `<span class="strong">${index + 1}.</span> ${
        item.label
      }`;
      ...
      customSelectEl.addEventListener("click", (e) => {
        const selectFlags = this.selectIsOpenedFlags.find(
        ...
        this.selectIsOpenedFlags = this.selectIsOpenedFlags.map((flag) => {
          if (flag.name === item.name)
            return { name: flag.name, isOpened: !flag.isOpened };
          return flag;
        });
      });
